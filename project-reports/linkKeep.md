...
- Aucune configuration CI/CD n'est presente dans le depot (pas de `.github/workflows/`, pas de `fastlane/`).
- Le site web dispose d'un `Dockerfile` production-ready pour un deploiement containerise.

## Installation et execution (local)

### Prerequis

- **Application** : Xcode (version supportant Swift 6.2, iOS 26 / macOS 26 SDK), macOS.
- **Site web** : Node.js, pnpm.

### Application Swift

```sh
# Build du package partage
cd application/Packages/LinkKeepCore
swift build
...
swift test --filter ServicesTests.BookmarkReminderCoordinatorTests

# Build de l'app iOS (depuis la racine du repo)
xcodebuild -project "application/LinkKeep-IOS/LinkKeep-IOS.xcodeproj" \
  -scheme "LinkKeep" \
  -destination 'platform=iOS Simulator,name=iPhone 17' \
  build

# Build de l'app macOS
xcodebuild -project "application/LinkKeep-MacOS/LinkKeep-MacOS.xcodeproj" \
  -scheme "LinkKeep" build
```
...
# Build Docker
docker build -t linkkeep-web .
docker run -p 3000:3000 linkkeep-web
```

## Limites connues et pistes d'amelioration

- **Pas de tests web** : l'absence de framework de test (Vitest, Playwright) est un risque pour le site web, surtout avec 9 locales a maintenir. L'ajout de tests de regression visuelle sur les composants serait un investissement raisonnable.
- **Pas de CI/CD** : aucune pipeline d'integration continue. Un GitHub Actions avec `swift test`, `pnpm check`, et `pnpm build` ameliorerait significativement la confiance dans les contributions.
- **Pas de lint/format automatique** : ni SwiftLint/SwiftFormat pour l'app, ni ESLint/Prettier pour le web. L'ajout d'outils de formatage automatique reduirait la charge cognitive dans les reviews.
- **Pas de file d'attente persistante pour les exports** : l'export genere un fichier temporaire ecrase a chaque appel. Pour de gros volumes, un systeme de file d'attente avec notification serait plus robuste.
- **Modele freemium simpliste** : le quota est verifie uniquement a la creation. Un utilisateur qui downgrade de Pro a Free pourrait conserver plus de signets que la limite. Une politique de downgrade (lecture seule au-dela du quota) serait plus coherente.
- **Share extension minimale** : l'extension iOS de partage existe mais ne propose ni selection de categorie ni de dossier a l'ajout. L'enrichir ameliorerait le workflow de capture.
- **SEO web avance** : les hreflang alternats sont codes en dur dans `seo-head.svelte` plutot que generes dynamiquement depuis la config Paraglide. Un mapping automatique eviterait la duplication.

## Liens

- **Depot local** : `/Users/sr-71/Documents/portfolio/repos_to_process/linkKeep`
